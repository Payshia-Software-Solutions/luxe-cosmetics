...
function DateWiseSale() {
  ...
  function fetch_data() {
    $.ajax({
      url: "assets/content/reporting_module/date-wise-sale-report/index.php",
      method: "POST",
      data: {
        LoggedUser: LoggedUser,
        UserLevel: UserLevel,
...
function PrintDateWiseSaleReport() {
  var fromQueryDate = document.getElementById("from-date-input").value;
  var toQueryDate = document.getElementById("to-date-input").value;
  var location_id = document.getElementById("location_id").value;
  // Open a new tab with the printPage.html and pass the po_number as a query parameter
...
async function PrintReceiptReportNew() {
  ...
  var encryptedData = await encryptData(
    { location_id, fromQueryDate, toQueryDate },
    key,
    iv
  );
...
async function encryptData(data, key, iv) {
  try {
    ...
    // Encrypt the data using AES-GCM
    var encryptedDataBuffer = await crypto.subtle.encrypt(
    ...
    return btoa(
      String.fromCharCode.apply(null, new Uint8Array(encryptedDataBuffer))
    );
  } catch (error) {
...
async function decryptData(encryptedData, key, iv) {
  try {
    ...
    var encryptedDataBuffer = new Uint8Array(
      atob(encryptedData)
        .split("")
    ...
    var decryptedDataBuffer = await crypto.subtle.decrypt(
      { name: "AES-GCM", iv: ivArray },
      importedKey,
      encryptedDataBuffer
    );
