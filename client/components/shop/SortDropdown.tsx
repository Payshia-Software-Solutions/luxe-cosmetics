...
const SortDropdown: React.FC<SortDropdownProps> = ({ onChange, currentSort }) => {
  ...
  const currentSortLabel = sortOptions.find(option => option.value === currentSort)?.label || "Sort Products";
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center justify-between w-full px-4 py-3 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors duration-200"
      >
        <div className="flex items-center">
          <FaSort className="text-gray-400 mr-2" size={14} />
          <span className={`${italiana.className} text-gray-700`}>{currentSortLabel}</span>
        </div>
        <span>
